...
async function jiraGetIssue(issueIdOrKey) {
  return await makeRequest(`/rest/api/3/issue/${issueIdOrKey}`);
}
...
async function jiraCreateIssue(projectKey, summary, description, issueType, extraFields = {}) {
  ...
  return await makeRequest('/rest/api/3/issue', 'POST', body);
}

async function jiraUpdateIssue(issueIdOrKey, fields) {
  // Convert description to ADF if present
  if (fields.description) {
    fields.description = toADF(fields.description);
  }
  const body = { fields };
  return await makeRequest(`/rest/api/3/issue/${issueIdOrKey}`, 'PUT', body);
}

async function jiraAddComment(issueIdOrKey, body) {
  return await makeRequest(`/rest/api/3/issue/${issueIdOrKey}/comment`, 'POST', {
    body: toADF(body)
  });
}

async function jiraGetTransitions(issueIdOrKey) {
  return await makeRequest(`/rest/api/3/issue/${issueIdOrKey}/transitions`);
}

async function jiraTransition(issueIdOrKey, transitionId) {
  return await makeRequest(`/rest/api/3/issue/${issueIdOrKey}/transitions`, 'POST', {
    transition: { id: transitionId }
  });
}

async function jiraLinkIssues(inwardIssueKey, outwardIssueKey, linkType) {
  return await makeRequest('/rest/api/3/issueLink', 'POST', {
    type: { name: linkType },
...
async function jiraAddWorklog(issueIdOrKey, timeSpent, comment = '', started = null) {
  ...
  return await makeRequest(`/rest/api/3/issue/${issueIdOrKey}/worklog`, 'POST', body);
}
...
async function jiraSearchProjects(query, maxResults = 50) {
  return await makeRequest(`/rest/api/3/project/search?query=${encodeURIComponent(query)}&maxResults=${maxResults}`);
}

async function jiraGetProjectIssueTypes(projectIdOrKey) {
  return await makeRequest(`/rest/api/3/issue/createmeta/${projectIdOrKey}/issuetypes`);
}
      ...
      return {
        ...
        result: {
          ...
          serverInfo: {
            ...
            version: '1.1.0',
            description: 'Jira Cloud MCP (API v3)'
          }
        }
      };
          ...
          tools: [
            {
              ...
              inputSchema: {
                type: 'object',
                properties: {
                  query: { type: 'string', description: 'Query string to match project name or key' },
            ...
            },
            {
              ...
              description: 'Get issue types (work types) available for a project',
              inputSchema: {
                type: 'object',
                properties: {
    ...
    case 'tools/call': {
      ...
      switch (toolName) {
        case 'jira_search_projects':
          result = await jiraSearchProjects(args.query, args.maxResults);
          break;
 ...
 */
async function main() {
  console.error(`Jira Cloud MCP Server (v3) started`);
